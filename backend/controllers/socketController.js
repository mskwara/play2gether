    ...
    return async (data) => {
        ...
        if (user && convs.includes(data.room)) {
            ...
            if (process.env.NODE_ENV === "development")
                console.log(
                    `User ${user._id} succesfully connected to chat room no. ${data.room}.`
                );
...
function leave(socket) {
    return async (data) => {
        ...
        if (user && convs.includes(data.room)) {
            ...
            if (process.env.NODE_ENV === "development")
                console.log(
                    `User ${user._id} succesfully disconnected from chat room no. ${data.room}.`
                );
        } else if (process.env.NODE_ENV === "development") {
...
function send(io) {
    return async (data) => {
        ...
        if (user && convs.includes(data.room) && data.message !== "") {
            ...
            if (process.env.NODE_ENV === "development") {
                console.log(
                    `User ${user._id} succesfully sent message to chat room no. ${data.room}.`
                );
            }
        } else if (process.env.NODE_ENV === "development") {
